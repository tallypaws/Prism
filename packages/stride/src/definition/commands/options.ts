...
import type { GuildCacheReducer, GuildValue } from "../../util.js";
...
/** A base option. */
export type BaseOption = { description: GuildValue<string>; required?: GuildValue<boolean> };
...
> {
	const transformedOptions = await Promise.all(
		Object.entries(options).map(async ([name, option]) => {
			const transformed = {
				name,
				description: await resolveGuildValue(option.description, guildId),
				type: option.type,
				required: (await resolveGuildValue(option.required ?? false, guildId)) ?? false,
			} as Exclude<
				ApplicationCommandOptionData,
				{
					type:
			...
			if (transformed.type === ApplicationCommandOptionType.String) {
				...
				if (option.maxLength !== undefined)
					transformed.maxLength = await resolveGuildValue(option.maxLength, guildId);
				if (option.minLength !== undefined)
					transformed.minLength = await resolveGuildValue(option.minLength, guildId);
			}
			...
			if (
				...
				if (option.maxValue !== undefined)
					transformed.maxValue = await resolveGuildValue(option.maxValue, guildId);
				if (option.minValue !== undefined)
					transformed.minValue = await resolveGuildValue(option.minValue, guildId);
			}
	...
	return transformedOptions.sort((one, two) =>
