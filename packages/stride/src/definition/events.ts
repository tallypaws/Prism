...
export type StrifeEvents = Exclude<
  keyof ClientEvents,
  (typeof reservedClientEvents)[number]
>;
...
export const allEvents: {
  [EventName in StrifeEvents]?: EventHandler<EventName>[];
} = {};
/** An object containing all registered pre-event handlers. */
export const preEvents: {
  [EventName in StrifeEvents]?: PreEventHandler<EventName>;
} = {};
...
export function defineEvent<EventName extends StrifeEvents>(
  ...
  if (!loginActive)
    throw new ReferenceError("Events cannot be defined after login");
...
defineEvent.pre = function pre<EventName extends StrifeEvents>(
  ...
  if (preEvents[eventName])
    throw new ReferenceError(
      `Pre-event handler for event ${eventName} already exists`,
    );
  // @ts-expect-error TS2322
    ...
    async function handler(...args: ClientEvents[StrifeEvents]): Promise<void> {
      ...
      if (failures.length)
        throw new AggregateError(
          failures,
          `Errors in multiple ${eventName} handlers`,
        );
    }
